       module mod_poscar
         ...
         character*2,allocatable,public::ele_name(:)
         logical,public              :: L_SELECT_DYN,cartesian,vasp4
         logical,allocatable,public  :: dynmics(:,:)
...
!          write(*,'(3X,A,f16.8)')"Volume of cell:",omega
           if(omega<=0.0)then
             write(*,'(3X,A)')"ERROR in lattice cell. Stop!!!"
             stop
           endif
!          write(*,*)
...
!              write(*,'(5X,3f16.8)')relatt(1:3,i)
           ...
           if (tchar>='0' .and. tchar<='9') then ! if this is a number its vasp4.*
              call num_elements(line1,255,ntype)  ! now the number of atom sorts in the POSCAR
           else                                  ! else it is vasp5.* and one line has to be discarded
              vasp4=.FALSE.
              read(2,'(A)') line2
              call num_elements(line2,255,ntype)  ! now the number of atom sorts in the POSCAR
           endif
!          write(*,*)
!          write(*,'(3X,A,I5,A)')'Found ',ntype,' atom type(s)'
           allocate(nitype(ntype),ele_name(ntype))
           if(vasp4)then
             read(line1,*)(nitype(i),i=1,ntype)
           else
             read(line1,*)(ele_name(i),i=1,ntype)
             read(line2,*)(nitype(i),i=1,ntype)
           endif
...
!             write(*,'(3X,A)')'original pos in direct lattice'
           ...
           do i=1,nions
              if(L_SELECT_DYN)then
                read(2,*)pos(1:3,i),dynmics(1:3,i)
              else
                read(2,*)pos(1:3,i)
              endif
           enddo
...
!             write(*,'(5X,I3,3f12.6,3X,3f12.6)')i,pos(1:3,i),posca(1:3,i)
           ...
           do n=1,nions
             do i=n+1,nions
                if( all( abs( posca(:,n) - posca(:,i) ) < 1E-3) ) then
