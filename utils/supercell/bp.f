! Generate a supercell out of a generating cell
      ...
      INTEGER     :: SHIFT_SC_A(3),SHIFT_PC_A(3,3)!jx-add
...
! If not store this position
         IF (.NOT.LFOUND) THEN
            ...
            id_map(1,NCOUNT) = I1-(SHIFT_PC_A(1,1)+SHIFT_PC_A(2,1)+&
            SHIFT_PC_A(3,1))
            id_map(2,NCOUNT) = I2-(SHIFT_PC_A(1,2)+SHIFT_PC_A(2,2)+&
            SHIFT_PC_A(3,2))
            id_map(3,NCOUNT) = I3-(SHIFT_PC_A(1,3)+SHIFT_PC_A(2,3)+&
            SHIFT_PC_A(3,3))
         ENDIF
        ENDDO
       ENDDO
