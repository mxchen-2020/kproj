...
plt.figure(figsize=(10,6.18))               #picture-size：x:y
plt.xlim(KPOINT[0], KPOINT[NKPOINT-1])    #x-axis(KPOINT RANGE)
plt.ylim( -25, 10)                         #y-axis(ENERGY RANGE)
plt.axhline(y=0,ls="--",c="black")   #E-Fermi
factor = 50                               #Control the size of the point           ↓
plt.scatter(KPOINT_PLOT,EIGENV_PLOT,alpha=WEIGHT_PLOT,c=WEIGHT_PLOT,cmap="Greys",s=WEIGHT_PLOT*factor,marker='o',vmin=0,vmax=1)
for i in range(2):                   #High SYMMETRY POINT
    plt.axvline(x=KSPLIT_POS[i],ls="--",c='black',alpha=0.5)
