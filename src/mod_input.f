...
!----------------------------------------------------other unfolding
       INTEGER,PUBLIC                  :: ions_type,orbital_tot,det
       INTEGER,ALLOCATABLE,PUBLIC      :: ions_n(:) , orbital_n(:) ,proj_type(:)    !LCAO INPUT
       INTEGER,PUBLIC                  :: NKPOINT, NSPIN                 !TOT NUMBER
       REAL(DP),ALLOCATABLE,PUBLIC     :: KPOINT_DIRECT(:,:),KPOINT_CARTESIAN(:,:)
       REAL(DP),ALLOCATABLE,PUBLIC     :: POS_MAP_CARTESIAN(:,:,:),POS_MAP_DIRECT(:,:,:)
    ...
    & 'Error reading item ''NSPIN'' from file INKPROJ.'
     ...
     & 'error in reading item "ions_n" from file INKPROJ.'
         ...
         allocate(proj_type(ions_type))
         proj_type=0
        call rdatab(lopen,'INKPROJ',IUI,'proj_type', '=', '#', ';', 'I', &
     &     proj_type(1), RDUM, CDUM, LDUM, CHARAC, N, ions_type, IERR)
         if ( ( (IERR/=0) .and. (IERR/=3) ).or. &
     &      ((IERR==0).and.(N<1))) then
            write(IUO,'(5X,A)') &
     & 'error in reading item "proj_type" from file INKPROJ.'
         endif
   ...
   OPEN(UNIT=9,FILE="pos_map.dat")
      ...
      DO I = 1,ions_type
        DO J = 1,ions_n(I)
          ...
          POS_MAP_CARTESIAN(I,J,1)=POS_MAP_DIRECT(I,J,1)*CELL_P(1,1)+POS_MAP_DIRECT(I,J,2)*CELL_P(2,1)+POS_MAP_DIRECT(I,J,3)*CELL_P(3,1)
          POS_MAP_CARTESIAN(I,J,2)=POS_MAP_DIRECT(I,J,1)*CELL_P(1,2)+POS_MAP_DIRECT(I,J,2)*CELL_P(2,2)+POS_MAP_DIRECT(I,J,3)*CELL_P(3,2)
          POS_MAP_CARTESIAN(I,J,3)=POS_MAP_DIRECT(I,J,1)*CELL_P(1,3)+POS_MAP_DIRECT(I,J,2)*CELL_P(2,3)+POS_MAP_DIRECT(I,J,3)*CELL_P(3,3)
        ENDDO
      ENDDO
     CLOSE(9)
