...
!   INTEGER,ALLOCATABLE      :: seg(:)
   ...
   INTEGER                  :: ICOUNT
   !-----------PHONON
   COMPLEX(DP),ALLOCATABLE  :: C_PHONON(:,:,:,:,:,:),C(:)
   INTEGER                  :: IDIR,ions_tot,natom
   REAL(DP),ALLOCATABLE     :: POSCAR_C(:,:),CONTCAR_C(:,:),POSCAR_D(:,:),CONTCAR_D(:,:),proj_wt_part(:,:,:,:)
...
!        DO I =1,ions_tot
!        READ(13,*)CONTCAR_D(I,:)
!        ENDDO    inquire (file='pos_map.dat', exist=ex)
!    IF (ex == .false.) then
...
read(17,"(6x,I)")npath
NKPOINT=NKPTS
!ALLOCATE(C_PHONON(NKPOINT,NBAND,part,maxval(ions_type),maxval(ions_n),3))
!allocate(seg(npath))
!ALLOCATE(proj_wt(NBAND,NKPOINT,1),CELTOT(NBAND,NKPOINT,1),proj_wt_part(part,NBAND,NKPOINT,1))
...
read(17,"(6x,I)")natom
read(17,*)
read(17,"(7X,F,F,F18.15)")CELL(1,1),CELL(1,2),CELL(1,3)
read(17,"(7X,F,F,F18.15)")CELL(2,1),CELL(2,2),CELL(2,3)
read(17,"(7X,F,F,F18.15)")CELL(3,1),CELL(3,2),CELL(3,3)
read(17,*)
do i=1,(3*natom)+2
read(17,*)
enddo
...
read(17,"(18X,F,F,F10.7)")KPTVEC(1,IKPOINT),KPTVEC(2,IKPOINT),KPTVEC(3,IKPOINT)
...
do I = 1,natom
   ...
   do IDIR=1,3
      ...
      C(ICOUNT)=cmplx(real_vector,im_vector)
      ICOUNT=ICOUNT+1
   enddo
enddo
...
do I = 1,part
   do J = 1,ions_type(I)
      do K = 1,ions_n(I,J)
         do IDIR=1,3
            C_PHONON(IKPOINT,IBAND,I,J,K,IDIR)=C(ICOUNT)
            ICOUNT=ICOUNT+1
         enddo
      enddo
   enddo
      ...
      DO IBAND = 1,NBAND 
         DO I= 1,part
            DO J = 1,ions_type(I)
               DO IDIR = 1, 3 !xyz
                  DO K= 1, ions_n(I,J)
                     WEIGHT_TEMP=WEIGHT_TEMP+C_PHONON(IKPOINT,IBAND,I,J,K,IDIR)
                  ENDDO
                  proj_wt_part(I,IBAND,IKPOINT,1) = WEIGHT_TEMP*CONJG(WEIGHT_TEMP)+proj_wt_part(I,IBAND,IKPOINT,1)
...
!--->    break up the k-points into lines, assuming that the order is correct
     ...
     &       nkpts0,ISPIN,ef
      ...
      write(bskfh,'(6f12.6)') x_kpt( segment_endpoint(1:kpt_lines) )/AUTOA !distance( seg(1:npath) )
     ...
     &      kpt,NBANDS,jspin,bkpts(:,ikpt),x_kpt(ikpt)/AUTOA!distance(ikpt)
          do nb = 1,NBANDS
