     ...
     !
      DO IBAND = 1,NBAND!NBAND
         ...
         DO I = 1,part
            DO J = 1 , ions_type(I)
               DO K = 1, ions_n(I,J)
                  DO L = 1,orbital_n(I,J)  !????还没增加分配轨道的功能
                     DO ISPIN = 1,NSPIN
...
SUBROUTINE read_nscf_log
    ...
    if (index(trim(line), 'atom label =') > 0) then
            L_count=1
            line = trim(adjustl(line))
            read(line,'(13x,A)')ele_list(type_loop)%ele_name
            do 
            read(13, '(A)', iostat=iostat) line
            line = trim(adjustl(line))
            ...
            !cal  orbital_num
            do i =1 ,4
            ele_list(type_index)%orbital_num=(2*L(i,1)+1)*L(i,2)+ele_list(type_index)%orbital_num
            enddo
